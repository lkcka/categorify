...
точек входа (CLI evaluate.py, позже — REST API), которые сами создают
MockLLMClient(), если пользователь ничего не настроил. Это стандартное
разделение "чистый механизм / дефолт продукта".

Батчинг: categorize_batch() — основной метод для реальной обработки
выписки (используется evaluate.py и будущим REST-эндпоинтом). Одиночный
categorize() — тонкая обёртка над батчем из одного элемента, подходит
для API "категоризировать одну транзакцию", но неэффективна как основной
путь при больших объёмах — так и задумано.
